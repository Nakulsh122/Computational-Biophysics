...
output_file = open("A1_22CS10046.txt", "w+")
...
def extract_pdb_sequences(pdb_file, pdb_id):
    ...
    if chain_breaks:
        for chain_id, breaks in chain_breaks.items():
            print(
                f"Chain Break Detected in {pdb_id}|Chains: {chain_id}, Residue positions {breaks}"
            )
    else:
...
def get_asa_values(pdb_filename):
    ...
    asa_filename = pdb_filename.replace(".pdb", ".asa")  
    asa_values = {}
...
# Length of chains 
def get_length(pdb_sequences):
    AA_S = set("ACDEFGHIKLMNPQRSTVWY")
    for chain_id, sequence in pdb_sequences.items():
        valid_length = sum(1 for aa in sequence if aa in AA_S)
        print(f"Chain ID: {chain_id} | Chain Length: {valid_length}")
...
if __name__ == "__main__":
    ...
    # Alignment Results
    results = align_sequence_clusters(
        fasta_clusters=fasta_seq, pdb_clusters=pdb_sequences
    )
    ...
    # BLOSUM Scores
    blosum_scores = calculate_blosum_scores(
        pdb_dict=pdb_sequences, fasta_dict=fasta_seq
    )
    ...
    for chain_id, seq in pdb_sequences.items():
        print(
            f"Chain {chain_id}: Molecular Weight = {calculate_molecular_weight(seq):.2f} Da"
        )
    # Calculate Length of chains
    print("\nCalculating Chain Lengths")
    print("\n------------------------------------------------")
    lenghts = get_length(pdb_sequences)
    print("\nRun Successful")
    print("\n------------------------------------------------")
